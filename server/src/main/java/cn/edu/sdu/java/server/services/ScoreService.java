...
public class ScoreService {
    ...
    public DataResponse getScoreList(DataRequest dataRequest) {
        ...
        List<Score> sList = switch (roleName){
            case "ROLE_STUDENT" -> scoreRepository.findByStudentCourse(userId,courseId);
            case "ROLE_TEACHER" -> scoreRepository.findByStudentCourse(personId,courseId);
            case "ROLE_ADMIN" -> scoreRepository.findByStudentCourse(personId,courseId);
            default -> null;
        }; //数据库查询操作
