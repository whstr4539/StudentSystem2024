...
public class StudentLeaveService {
    ...
    public DataResponse studentLeaveCheck(DataRequest dataRequest) {
        ...
        if("ROLE_ADMIN".equals(roleName)) {
            if(sl.getState()==3)return CommonMethod.getReturnMessage(1,"教师审核未通过");
            if(sl.getState()==0)return CommonMethod.getReturnMessage(1,"学生未提交申请");
            if(sl.getState()==1)return CommonMethod.getReturnMessage(1,"教师未审核");
